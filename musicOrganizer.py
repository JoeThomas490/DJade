...
destinationDirectory = StringVar()
...
def chooseWorkingDir():
    ...
    workingDirectory_entry.delete(0, END)
    workingDirectory_entry.insert(0, workingDirectory)
    saveData({"workingDirectory" : workingDirectory_entry.get(), "destinationDirectory" : destinationDirectory_entry.get()})
    print ("Changed working directory to : " + workingDirectory)

def chooseDestinationDir():
    destinationDirectory = filedialog.askdirectory()
    destinationDirectory_entry.delete(0,END)
    destinationDirectory_entry.insert(0, destinationDirectory)
    saveData({"workingDirectory" : workingDirectory_entry.get(), "destinationDirectory" : destinationDirectory_entry.get()})
    print("Changed destination directory to : " + destinationDirectory)
...
workingDirectory_entry = ttk.Entry(mainframe, width = 120, textvariable = workingDirectory)
workingDirectory_entry.grid(column=0, row=0, sticky=(N,S, W, E))
workingDirectory_entry.columnconfigure(0, weight=1)
workingDirectory_entry.rowconfigure(0, weight=1)

chooseWorkingDirectory_btn = ttk.Button(mainframe, text="Choose working directory", command = lambda:chooseWorkingDir())
chooseWorkingDirectory_btn.grid(column=1, row=0, sticky=(N,E))

destinationDirectory_entry = ttk.Entry(mainframe, width = 120, textvariable = destinationDirectory)
destinationDirectory_entry.grid(column=0, row=1, sticky=(N,S,W,E))

destinationDirectory_btn = ttk.Button(mainframe, text="Choose destination", command = lambda:chooseDestinationDir())
destinationDirectory_btn.grid(column=1, row=1, sticky=(N,S,W,E))
